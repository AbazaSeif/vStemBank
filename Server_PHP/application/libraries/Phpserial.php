    ...
    public function __construct() {
        ...
        if (substr($sysname, 0, 5) === "Linux") {
            ...
            if ($this->_exec("stty --version") === 0) {
                ...
                return "No stty availible, unable to run.";
            }
...
// 		     }
            ...
            return "Host OS is neither osx, linux nor windows, unable to run.";
        }
    }
        ...
        if ($this->_dState !== SERIAL_DEVICE_OPENED) {
            if ($this->_os === "linux") {
                ...
                if ($this->_exec("stty -F " . $device) === 0) {
                    ...
                    return "Could not connect with Bluetooth Device";
                }
            } elseif ($this->_os === "osx") {
                if ($this->_exec("stty -f " . $device) === 0) {
                    $this->_device = $device;
            ...
            echo "Specified serial port is not valid";
        } else {
            echo "You must close your device before to set an other one";
        }
    }
    ...
    public function deviceOpen($mode = "r+b") {
        ...
        if ($this->_dState === SERIAL_DEVICE_NOTSET) {
            return "The device must be set before to be open";
        }

        if (!preg_match("@^[raw]\+?b?$@", $mode)) {
            return "Invalid opening mode : " . $mode . ". Use fopen() modes.";
        }
    ...
    public function deviceClose() {
        ...
        return "Unable to close the device";
    }
    ...
    public function confBaudRate($rate) {
        if ($this->_dState !== SERIAL_DEVICE_SET) {
            return "Unable to set the baud rate : the device is either not set or opened";
        }
        ...
        if (isset($validBauds[$rate])) {
            ...
            if ($ret !== 0) {
                return("Unable to set baud rate: " . $out[1]);
            } else
                return true;
        } else
            return("Unalbe to set baud rate: " . $rate);
    }
    ...
    public function confParity($parity) {
        if ($this->_dState !== SERIAL_DEVICE_SET) {
            return("Unable to set parity : the device is either not set or opened");
        }
        ...
        if (!isset($args[$parity])) {
            return("Parity mode not supported");
        }
        $out = null;
        if ($this->_os === "linux") {
        ...
        return("Unable to set parity : " . $out[1]);
    }
    ...
    public function confCharacterLength($int) {
        if ($this->_dState !== SERIAL_DEVICE_SET) {
            return("Unable to set length of a character : the device is either not set or opened");
        }
        ...
        return("Unable to set character length : " . $out[1]);
    }
    ...
    public function confStopBits($length) {
        if ($this->_dState !== SERIAL_DEVICE_SET) {
            return("Unable to set the length of a stop bit : the device is either not set or opened");
        }

        if ($length != 1 and $length != 2 and $length != 1.5 and ! ($length == 1.5 and $this->_os === "linux")) {
            return("Specified stop bit length is invalid");
        }
        $out = null;
        if ($this->_os === "linux") {
        ...
        return("Unable to set stop bit length : " . $out[1]);
    }
    ...
    public function confFlowControl($mode) {
        if ($this->_dState !== SERIAL_DEVICE_SET) {
            return("Unable to set flow control mode : the device is either not set or opened");
        }
        ...
        if ($mode !== "none" and $mode !== "rts/cts" and $mode !== "xon/xoff") {
            return("Invalid flow control mode specified");
        }
        $out = null;
        if ($this->_os === "linux")
        ...
        if ($ret === 0)
            ...
            return("Unable to set flow control : " . $out[1]);
        }
    }
    ...
    public function setSetserialFlag($param, $arg = "") {
        ...
        if ($return{0} === "I") {
            return("setserial: Invalid flag");
        } elseif ($return{0} === "/") {
            return("setserial: Error with device file");
        } else {
            return true;
        }
    }
    ...
    public function readPort($count = 0) {
        if ($this->_dState !== SERIAL_DEVICE_OPENED) {
            return("Device must be opened to read it");
        }
